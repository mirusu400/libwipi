---
myst:
  html_meta:
    description: "Open a fixed-record database and optionally create it."
---

<!-- Generated by tools/generate_docs.py. Do not edit. -->

# `MC_dbOpenDataBase`

Open a fixed-record database and optionally create it.

## Prototype

```c
M_Int32 MC_dbOpenDataBase(M_Char *dataBaseName, M_Int32 recordSize, M_Boolean create, M_Int32 mode);
```

```{doxygenfunction} MC_dbOpenDataBase
:project: libwipi
```

## Catalog identity

| Field | Value |
|---|---|
| API level | `1.2.1` |
| Family | [`MC_DB`](../families/database.md) |
| Ordinal | 125 |
| Documentation | `draft` |
| Implementation class | `table` |
| ABI class | `word-tail` |

## Usage

### Parameters

| Name | Meaning |
|---|---|
| `dataBaseName` | Database name. |
| `recordSize` | Fixed record size. |
| `create` | Whether the database may be created. |
| `mode` | Database access mode. |

### Return value

A database handle or the provider-defined failure value.

## Profile availability

API membership, device ABI, and install profile are independent. The rows below
describe libwipi implementation evidence, not platform-wide WIPI requirements.

| ABI profile | Install profile | Current evidence |
|---|---|---|
| `host-sim` | `none` | headers only |
| `ktf-samsung` | `none` | linkable generated veneer |
| `ktf-samsung` | `aram-ktf` | linkable generated veneer |
| `ktf-samsung` | `sch-w8300-qpst-probe` | linkable generated veneer; named-device candidate |
| `lgt-raptor` | `aram-raptor` | observed in the scoped emulator suite |
| `lgt-raptor` | `aram-wie-raptor` | declared; no confirmed adapter |
| `skt-samsung-sch-w830-dl21` | `none` | declared; no direct SCH binding |

## Compiled examples

- [Database CRUD](../../../examples/database-crud.md)

## Evidence boundary

Samsung/KTF selector confidence: `confirmed`. Catalog evidence: `anycall_magichole:analysis/decompiled/wipi_mc_db_spec_adapter.c:234`.
